...
// 1.
dogs.forEach(dog => {
  const recommendedFoodInGrams = Math.trunc(dog.weight ** 0.75 * 28);
  dog.recommendedFoodInGrams = recommendedFoodInGrams;
});
console.log(dogs);
...
console.log(
  sarahsDog.curFood > sarahsDog.recommendedFoodInGrams * 1.1
    ? `Too much`
    : `Too little`,
);
...
const ownersTooMuch = dogs
  .filter(dog => dog.curFood > dog.recommendedFoodInGrams * 1.1)
  .flatMap(dog => dog.owners);
const ownersTooLittle = dogs
  .filter(dog => dog.curFood < dog.recommendedFoodInGrams * 0.9)
  .flatMap(dog => dog.owners);
...
console.log(
  dogs.some(
    dog =>
      dog.curFood > dog.recommendedFoodInGrams * 0.9 &&
      dog.curFood < dog.recommendedFoodInGrams * 1.1,
  ),
);
...
// 7.
const dogsWitchHaveGoodEating = dogs.filter(
  dog =>
    dog.curFood > dog.recommendedFoodInGrams * 0.9 &&
    dog.curFood < dog.recommendedFoodInGrams * 1.1,
);
console.log(dogsWitchHaveGoodEating);

// 8.
const groupedDogsByEating = Object.groupBy(dogs, dog => {
  if (dog.curFood > dog.recommendedFoodInGrams * 1.1) return `too-much`;
  if (dog.curFood < dog.recommendedFoodInGrams * 0.9) return `too-little`;
  return `exact`;
});
console.log(groupedDogsByEating);

// 9.
const groupedDogsByNumberOwners = Object.groupBy(
  dogs,
  dog => dog.owners.length,
);
console.log(groupedDogsByNumberOwners);

// 10.
const sortedDogsByRecommendedFood = dogs.toSorted(
  ({ recommendedFoodInGrams: a }, { recommendedFoodInGrams: b }) => a - b,
);
console.log(sortedDogsByRecommendedFood);
